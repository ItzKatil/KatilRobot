...
from KatilRobot import DRAGONS
from KatilRobot.modules.helper_funcs.telethn import IMMUNE_USERS, telethn
...
async def Katil_is_admin(chat_id: int):
    status = False
    Katil = await telethn.get_me()
    async for user in telethn.iter_participants(
        chat_id, filter=ChannelParticipantsAdmins
    ):
        if Katil.id == user.id:
